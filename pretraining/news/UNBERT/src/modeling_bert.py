...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.segment_embeddings = nn.Embedding(100, config.hidden_size)
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, segment_ids=None):
...
class BertModel(BertPreTrainedModel):
    ...
    Return:
        ...
        def piece_sum(embed, size):
            ...
            return y.index_add_(0, ind, embed).unsqueeze(0)
        if 1 == 0:
            sentence_embedding = torch.cat([piece_sum(embed, size) for embed, size in zip(sequence_output, sentence_ids)], 0)
        else: 
            dummy = sentence_ids.unsqueeze(2).expand(sentence_ids.size(0), sentence_ids.size(1), sequence_output.size(2))
            sentence_embedding = torch.gather(sequence_output, 1, dummy)
        sentence_outputs = self.encoder(
            sentence_embedding, 
            attention_mask=self.get_extended_attention_mask(sentence_mask, sentence_ids.size(), sentence_ids.device),
            head_mask=head_mask,
            encoder_hidden_states=encoder_hidden_states,
            encoder_attention_mask=encoder_extended_attention_mask,
            output_attentions=output_attentions,
